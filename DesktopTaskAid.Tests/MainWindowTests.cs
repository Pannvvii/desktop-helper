...
namespace DesktopTaskAid.Tests
{
    ...
    public class MainWindowTests
    {
        ...
        public void Setup()
        {
            ...
            Application.Current.Resources["PrimaryButtonStyle"] = new Style(typeof(Button));
        ...
        [Test]
        public void UpdateWelcomeIllustration_CallsWithoutException()
        {
            var wnd = new MainWindow();
            var mi = typeof(MainWindow).GetMethod("UpdateWelcomeIllustration", BindingFlags.Instance | BindingFlags.NonPublic);
            Assert.IsNotNull(mi);

            // Test light theme (default, no merged dictionaries with "dark")
            Application.Current.Resources.MergedDictionaries.Clear();
            Assert.DoesNotThrow(() => mi.Invoke(wnd, null));

            // Test dark theme detection by adding a dictionary with a mock source
            // containing "darkTheme" in its ToString representation
            var rdDark = new ResourceDictionary();
            // Don't set Source to avoid pack URI issues; just ensure it's present
            // The actual code checks Any(d => d.Source?.ToString().Contains("darkTheme.xaml"))
            // In test, we'll just verify the method doesn't throw
            Application.Current.Resources.MergedDictionaries.Add(rdDark);
            Assert.DoesNotThrow(() => mi.Invoke(wnd, null));
        }

        [Test]
        public void LifecycleHandlers_NoThrow()
        {
            var wnd = new MainWindow();
            var loaded = typeof(MainWindow).GetMethod("MainWindow_Loaded", BindingFlags.Instance | BindingFlags.NonPublic);
            var rendered = typeof(MainWindow).GetMethod("MainWindow_ContentRendered", BindingFlags.Instance | BindingFlags.NonPublic);

            Assert.DoesNotThrow(() => loaded.Invoke(wnd, new object[] { null, new RoutedEventArgs() }));
            Assert.DoesNotThrow(() => rendered.Invoke(wnd, new object[] { null, EventArgs.Empty }));
        }
    }
}
