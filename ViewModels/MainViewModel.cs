...
namespace DesktopTaskAid.ViewModels
{
    public class MainViewModel : ViewModelBase
    {
        ...
        public static ObservableCollection<TaskItem> _allTasks;
        ...
        public MainViewModel()
        {
            ...
            try
            {
                ...
                // Auto-adjust calendar to today's date if persisted selection is stale
                var today = DateTime.Today;
                if (_state?.Calendar != null)
                {
                    if (_state.Calendar.SelectedDate.Date != today)
                    {
                        LoggingService.Log($"Adjusting SelectedDate from {_state.Calendar.SelectedDate:yyyy-MM-dd} to {today:yyyy-MM-dd}", "INFO");
                        _state.Calendar.SelectedDate = today;
                    }
                    if (_state.Calendar.CurrentMonth.Year != today.Year || _state.Calendar.CurrentMonth.Month != today.Month)
                    {
                        LoggingService.Log($"Adjusting CurrentMonth from {_state.Calendar.CurrentMonth:yyyy-MM} to {today:yyyy-MM}", "INFO");
                        _state.Calendar.CurrentMonth = new DateTime(today.Year, today.Month, 1);
                    }
                }
                ...
                {
                    LoggingService.Log("Invalid PageSize in state. Resetting to 10.", "WARN");
                    _state.PageSize = 10;
                }
                if (_state.CurrentPage <= 0)
                {
                    LoggingService.Log("Invalid CurrentPage in state. Resetting to 1.", "WARN");
                    _state.CurrentPage = 1;
                }

                // Refresh daily timer tracking
        ...
        private void RefreshDisplayedTasks()
        {
            ...
            var startIndex = (CurrentPage - 1) * safePageSize;
            var pageTasks = sortedTasks.Skip(startIndex).Take(safePageSize).ToList();
        ...
        {
            // Force a rescan (user may have dropped the file after launching the app)
            var stateAfterRescan = _calendarImportService.TryAutoImportNow();
            ApplyCredentialState(stateAfterRescan);
            if (HasValidCredentials)
            {
                return true;
            }

            // Do NOT show file dialog anymore; just instruct the user.
            if (stateAfterRescan.Status == CredentialStatus.Missing)
            {
                ImportStatusMessage = "Add google-credentials.json in the same folder as the .exe, then click Import again.";
            }
            else if (stateAfterRescan.Status == CredentialStatus.Invalid)
            {
                ImportStatusMessage = stateAfterRescan.Message;
            }
            return false;
        }

        private Task<bool> PromptForCredentialsAsync()
        {
            // Legacy method no longer used; always returns false.
        ...
        {
            _state.Tasks = AllTasks.ToList();
            _state.CurrentPage = Math.Max(1, CurrentPage);
            _state.PageSize = Math.Max(1, PageSize);
